...
def create_2D_plots(n, generator=None, fname=None):
    ...
    if generator == None:
        ...
        D = gen_linearly_separable_data(w_gen, n)
        # print(D)
    else:
        D = generator()

    learned_W, iters = learn(D)
    print(learned_W, iters)
    ...
    plt.scatter(xPos, yPos, c='blue', label='+', s=0.1)
    plt.scatter(xNeg, yNeg, c='red', label='-', s=0.1)

    x = np.linspace(lower_lin_space, upper_lin_space, upper_lin_space - lower_lin_space + 1)
    y_learned = learned_m * x + learned_b

    learned_label = "perceptron: x_2 = {} * x_1 + {}".format(learned_m, learned_b)
    w_linreg = lin_reg(D)
    lr_m = -1 * w_linreg[1]/w_linreg[2]
    lr_b = -1 * w_linreg[0]/w_linreg[2]
    y_linreg = lr_m * x + lr_b
    linreg_label = "linear regression: x_2 = {} * x_1 + {}".format(lr_m, lr_b)

    plt.plot(x, y_learned, c='purple', label=learned_label)
    plt.plot(x, y_linreg, c='green', label=linreg_label)
